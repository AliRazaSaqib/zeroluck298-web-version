...
$(".custom-pagination").each(function () {
  ...
  if ($(".custom-pagination").length > 0) {
    ...
    // this_val
    //   .find(".item-pagination .page-count")
    //   .after('<span class="total-page-count"> / ' + noOfPageCount + "</span>");
    ...
    if (totalPagenum > 1) {
      ...
      for (var n = 1; n <= 1; n++) {
        this_val
          .find(".item-pagination .page-count b:nth-child(" + n + ")")
          .show();
      }
    } else {
...
function displayRows(this_current) {
  ...
  for (
    var k = currentPage * pageSize - (pageSize - 1);
    k <= currentPage * pageSize;
    k++
  ) {
    $(this_current)
      .find(".item-listing .item-single:nth-child(" + k + ")")
      .show();
  }
...
function displayevent(this_current) {
  $(this_current)
    .find(".item-pagination .page-count b")
    .each(function () {
